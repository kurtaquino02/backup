...
function loadOrderSummary() {
    ...
    cart.forEach(item => {
        ...
        // Build image content - if it's a URL, render as img, otherwise use emoji
        const imgUrl = item.image && item.image.startsWith('http') ? item.image : '';
        const imgContent = imgUrl 
            ? `<img src="${imgUrl}" alt="${item.name}" style="width:100%;height:100%;object-fit:cover;border-radius:8px;" onerror="this.onerror=null;this.parentElement.innerHTML='📦'">`
            : '📦';

        const itemEl = document.createElement('div');
        itemEl.className = 's-item';
        itemEl.innerHTML = `
            <div class="s-img">${imgContent}</div>
            <div class="s-info">

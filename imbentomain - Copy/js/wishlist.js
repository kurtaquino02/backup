...
function addAllToCart() {
  ...
  setTimeout(() => {
      ...
      updateCount();
      showToast(`🛒 Moved ${cards.length} items to cart!`, 'success');
...
function loadStoredWishlist() {
  ...
  stored.forEach(item => {
      ...
      // Image Logic - prioritize actual image, fallback to emoji/icon
      let imgContent = '';
      if(item.image && item.image.includes('/')) {
         // It's a path (e.g. products/foo.jpg)
         imgContent = `<img src="${item.image}" alt="${item.name}" style="width:100%;height:100%;object-fit:cover;" onerror="this.onerror=null;this.parentElement.innerHTML='<div style=\\'font-size:3rem;display:flex;justify-content:center;align-items:center;height:100%;width:100%\\'>📦</div>'">`;
      } else {
         // It's likely an emoji or undefined
         imgContent = `<div style="font-size:3rem;display:flex;justify-content:center;align-items:center;height:100%;width:100%">${item.image || '📦'}</div>`;
      }
      ...
      <div class="wish-img" style="background:linear-gradient(135deg,#F5F3FF,#DDD6FE)">
        ${imgContent}
        ${item.badge ? `<div class="wish-badges"><div class="wish-badge ${item.badge}">${item.badgeText || ''}</div></div>` : ''}
      ...
      <div class="wish-info">
        ...
        <div class="product-seller"><span class="seller-name">🏪 ${item.sellerName || 'Local Seller'}</span><span class="seller-loc">📍 ${item.sellerLocation || 'Philippines'}</span></div>
